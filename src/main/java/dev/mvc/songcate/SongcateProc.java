...
// 알고리즘 구현
@Service("dev.mvc.songcate.SongcateProc")
public class SongcateProc implements SongcateProcInter {
  @Autowired // CateDAOInter를 구현한 클래스의 객체를 자동으로 생성하여 cateDAO 객체에 할당
  private SongcateDAOInter songcateDAO;
  
  public SongcateProc() {
    System.out.println("-> SongcateProc created.");
  }
  
  @Override
  public int create(SongcateVO songcateVO) {
    int cnt = this.songcateDAO.create(songcateVO);
  ...
  public ArrayList<SongcateVO> list_all() {
    ArrayList<SongcateVO> list_all = this.songcateDAO.list_all();
  ...
  public SongcateVO read(Integer songcateno) {
    SongcateVO songcateVO = this.songcateDAO.read(songcateno);
  ...
  public int update(SongcateVO songcateVO) {
    int cnt = this.songcateDAO.update(songcateVO);
  ...
  public int delete(int songcateno) {
    int cnt = this.songcateDAO.delete(songcateno);
    return cnt;
  }
  
  @Override
  public int update_seqno_forward(int songcateno) {
    int cnt = this.songcateDAO.update_seqno_forward(songcateno);
    return cnt;
  }
  
  @Override
  public int update_seqno_backward(int songcateno) {
    int cnt = this.songcateDAO.update_seqno_backward(songcateno);
    return cnt;
  }
  
  @Override
  public int update_visible_y(int songcateno) {
    int cnt = this.songcateDAO.update_visible_y(songcateno);
    return cnt;
  }
  
  @Override
  public int update_visible_n(int songcateno) {
    int cnt = this.songcateDAO.update_visible_n(songcateno);
    return cnt;
  }
  
  @Override
  public ArrayList<SongcateVO> list_all_songcategrp_y(){
    ArrayList<SongcateVO> list = this.songcateDAO.list_all_songcategrp_y();
  ...
  public ArrayList<SongcateVO> list_all_songcate_y(String genre){
    ArrayList<SongcateVO> list = this.songcateDAO.list_all_songcate_y(genre);
 ...
 // 대분류+중분류의 결합
    ArrayList<SongcateVOMenu> menu = new ArrayList<SongcateVOMenu>();
    
    ArrayList<SongcateVO> genres = this.songcateDAO.list_all_songcategrp_y();
    for(SongcateVO songcateVO:genres) {
      SongcateVOMenu songcateVOMenu = new SongcateVOMenu();
      
//      System.out.println(songcateVO.getGenre());
      songcateVOMenu.setGenre(songcateVO.getGenre()); // 대분류명 저장
      
      // 카테고리 그룹(대분류)에 해당하는 카테고리 목록(중분류) 로딩
      ArrayList<SongcateVO> list_name = this.songcateDAO.list_all_songcate_y(songcateVO.getGenre()); 
      songcateVOMenu.setList_name(list_name);
      
      menu.add(songcateVOMenu);
    }
  ...
  public ArrayList<String> genreset() {
    ArrayList<String> list = this.songcateDAO.genreset();
    return list;
  }

  @Override
  public ArrayList<SongcateVO> list_search(String word) {
    ArrayList<SongcateVO> list = this.songcateDAO.list_search(word);
    return list;
  }

  @Override
  public Integer list_search_count(String word) {
    int cnt = this.songcateDAO.list_search_count(word);
    return cnt;
  }
   ...
   */
    ...
    ArrayList<SongcateVO> list = this.songcateDAO.list_search_paging(map);
...
//    str.append("현재 페이지: " + nowPage + " / " + totalPage + "  "); 
  ...
  public int cntcount(int songcateno) {
      return songcateDAO.cntcount(songcateno);
  }
